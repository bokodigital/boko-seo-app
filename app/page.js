    ...
    <>
      ...
      <div className="wrap">
        <div className="panel">
              ...
              <span>
                {counts.withIssues ? (
                  <>
                    ...
                    {TABS.find((t) => t.key === activeTab).label.toLowerCase()} have SEO meta issues to fix.
                  </>
                ) : (
                  <>All {items.length} {TABS.find((t) => t.key === activeTab).label.toLowerCase()} have healthy meta titles &amp; descriptions.</>
                )}
              </span>
            </div>
          )}
          ...
          {!loading && !loadError && items.length === 0 && (
            <div className="empty">No {TABS.find((t) => t.key === activeTab).label.toLowerCase()} found in this store.</div>
          )}
