...
export default function Page() {
  ...
  const importItem = useCallback(
    async (item) => {
      ...
      if (item.locked) {
        showToast("Upgrade with Boko to optimise this item.");
        return false;
      }
      patchItem(item.type, item.id, { status: "working", error: "" });
      try {
            ...
            {gate && gate.locked && (
              <div className="upgrade-banner">
                <div className="upgrade-copy">
                  <b>
                    {gate.siteNotice
                      ? "This " + "store" + " is on the free allowance."
    ...
    <div className={"card" + (item.status === "imported" ? " imported" : "")}>
      ...
      {showFields && (
        <>
          <div className="field">
            <label>
              Meta title <span className={"counter " + counterClass(item.genTitle, "title")}>{tLen} / {LIMITS.title.max}</span>
            </label>
            <textarea className="title" value={item.genTitle} onChange={(e) => onEdit("genTitle", e.target.value)} />
          </div>
          <div className="field">
            <label>
              Meta description <span className={"counter " + counterClass(item.genDesc, "description")}>{dLen} / {LIMITS.description.max}</span>
            </label>
